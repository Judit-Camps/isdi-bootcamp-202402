    ...
    describe('>helpers', () => {
        ...
        describe('_loadDocuments', () => {
            ...
            it('load existing array to new collection', done => {
                writeFile('./data/cars.json', '[{"brand":"seat","model":"ibiza"},{"brand":"VW","model":"golf"}]', error => {
                    if (error) {
                        console.error(error);
                        return;
                    }
    ...
    describe('>CRUD', () => {
        describe('findOne', () => {
            ...
            it('returns null on non-existing document', done => {
                writeFile('./data/cars.json', '[{"brand":"seat","model":"ibiza"}]', error => {
                    if (error) {
                        console.error(error);
                        return;
                    }
        ...
        describe('updateOne', () => {
            it('updates one document', done => {
                ...
                writeFile('./data/cars.json', docuJSON, error => {
                    if (error) {
                        console.error(error);
                        return;
                    }
                    ...
                    cars.updateOne(car => car.id === '123', newDoc, (error, updated) => {
                        ...
                        readFile('./data/cars.json', 'utf8', (error, documentJSON) => {
                            if (error) {
                                console.error(error);
                                return;
                            }
