...
function findEvents({ userId, organizationId, location, price, date, categories }: { userId: string, organizationId?: string, location?: string, price?: number, date?: string, categories?: string[] }): Promise<any> {
    ...
    return User.findById(userId)
        ...
        .then(user => {
            ...
            return query.exec()
                .catch(error => { throw new SystemError(error.message) })
                .then(events =>
                    events.map<{ id: string, author: { id: string, name: string }, title: string, city: string, address: string, date: Date, time: string, description: string, price: number, categories: string[] }>(({ _id, author, title, city, address, date, time, description, price, categories }) => ({
                        id: _id.toString(),
                        author: {
